...
#minimum 5%
temp <- sapply(cat.train, combine.levels, minlev = 0.001)
...
h2o.gbm.1 <- function(..., ntrees = 500, seed = 1) h2o.gbm.wrapper(..., ntrees = ntrees, seed = seed)
...
h2o.deeplearning.2 <- function(..., hidden = c(200,100,50), activation = "Tanh", epochs = 50, seed = 1) h2o.deeplearning.wrapper(..., hidden = hidden, activation = activation, seed = seed)
...
h2o.save_ensemble(fit, path = "fit_metagbm") # define your path here
...
pred_ensemble <-exp(as.matrix(pred[[1]]-shift))
...
submission$loss = as.numeric(as.matrix((exp(pred_test[[1]]-shift))))
write.csv(submission, 'h2_cd_ubuntu.csv', row.names=FALSE)
